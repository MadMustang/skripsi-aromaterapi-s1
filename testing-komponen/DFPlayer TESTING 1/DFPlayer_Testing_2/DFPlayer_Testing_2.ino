...
void loop()
{
  ...
  sendMP3Command(0x12, 0, 01); //play Specific Treck
...
void sendMP3Command(byte Command, byte Param1, byte Param2) {
  ...
  unsigned int checkSum = -(Version_Byte + Command_Length + Command + infoReq + Param1 + Param2);



  // Construct the command line

  byte commandBuffer[10] = { Start_Byte, Version_Byte, Command_Length, Command, infoReq, Param1, Param2, highByte(checkSum),
  ...
  for (int cnt = 0; cnt < 10; cnt++) {
    ...
    mySerial.write(commandBuffer[cnt]);
